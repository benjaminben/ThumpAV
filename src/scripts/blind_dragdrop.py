...
def onHoverStartGetAccept(comp, info):
	...
	"""
	try:
		return type(info["dragItems"][0]) == Channel
	except:
		return False
...
def onDropGetResults(comp, info):
	...
	try:
		chan = info["dragItems"][0]
		blindpar = op.CTRL.pars(f"Blind{comp.parent(2).digits}")[0]
		blindpar.expr = f"op('{chan.owner}')['{chan.name}']"
		blindpar.mode = ParMode.EXPRESSION
	except Exception as ex:
		debug("Error binding channel to blind:", ex)
...
def onDragStartGetItems(comp, info):
	...
	"""
	dragItems = [op(ipar.LiveLauncher).op('CTRL').pars(f'Blind{comp.parent(2).digits}')[0]] # drag the comp itself
